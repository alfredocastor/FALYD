...
<head>
    ...
    <title>Gestión de Maestros - Admin FALYD</title>
...
<body>
    ...
    <div class="sidebar d-flex flex-column">
        ...
        <nav class="nav flex-column flex-grow-1">
            ...
            <a class="nav-link active" href="admin_maestros.jsp"><i class="bi bi-person-badge-fill"></i> Gestión de Maestros</a>
    ...
    <div class="main-content">
        <%
            ...
            int totalDocentes = lista.size();
        %>
        
        <div class="d-flex justify-content-between align-items-center mb-5">
            <div>
                <h2 class="fw-bold mb-1" style="color: var(--blue-falyd);">Gestión de Maestros</h2>
                <p class="text-muted small mb-0">Administra la información de todos los maestros del sistema</p>
            </div>
            <div class="d-flex align-items-center">
                <button class="btn btn-primary fw-bold px-4 py-2 me-4 shadow-sm" data-bs-toggle="modal" data-bs-target="#modalNuevoMaestro" style="background-color: var(--blue-falyd); border: none; border-radius: 12px;">
                    <i class="bi bi-plus-lg me-2"></i>Agregar Maestro
                </button>
        ...
        <div class="row g-4 mb-5">
            <div class="col-md-3">
                <div class="metric-card border">
                    <div class="metric-icon-box" style="background-color: #e3f2fd; color: #1e88e5;"><i class="bi bi-person-badge-fill"></i></div>
                    <p class="text-muted small mb-1 fw-bold">Total de Maestros</p>
                    <h3 class="fw-bold text-primary mb-0"><%= totalDocentes %></h3>
                </div>
            </div>
            <div class="col-md-3">
                <div class="metric-card border">
                    <div class="metric-icon-box" style="background-color: #e8f5e9; color: #43a047;"><i class="bi bi-person-plus-fill"></i></div>
                    <p class="text-muted small mb-1 fw-bold">Nuevos Maestros</p>
                    <h3 class="fw-bold text-success mb-0">8</h3>
                </div>
            </div>
            <div class="col-md-3">
                <div class="metric-card border">
                    <div class="metric-icon-box" style="background-color: #e3f2fd; color: #0b3b60;"><i class="bi bi-person-check-fill"></i></div>
                    <p class="text-muted small mb-1 fw-bold">Maestros Activos</p>
                    <h3 class="fw-bold mb-0" style="color: var(--blue-falyd);">91.8%</h3>
                </div>
            </div>
            <div class="col-md-3">
                <div class="metric-card border">
                    <div class="metric-icon-box" style="background-color: #ffebee; color: #ef4444;"><i class="bi bi-person-x-fill"></i></div>
                    <p class="text-muted small mb-1 fw-bold">Maestros Inactivos</p>
                    <h3 class="fw-bold text-danger mb-0">7</h3>
                </div>
            </div>
        </div>

        <div class="card-custom">
            <h5 class="fw-bold mb-4"><i class="bi bi-people-fill me-2 text-primary"></i>Plantilla Docente Oficial</h5>
            
            <div class="table-responsive">
                <table class="table table-hover align-middle">
                    <thead>
                        <tr>
                            <th style="width: 120px;">ID Maestro</th>
                    ...
                    <tbody>
                        ...
                        %>
                            <tr>
                                <td colspan="5" class="text-center py-4 text-muted fw-bold">No hay maestros registrados en el sistema aún.</td>
                            </tr>
                        <%
                            } else {
                                for (Maestro m : lista) {
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="d-flex align-items-center">
                                    <img src="https://ui-avatars.com/api/?name=<%= m.getNombre() %>&background=e3f2fd&color=0b3b60" class="rounded-circle me-2" width="32">
                                    <span class="text-dark fw-bold"><%= m.getNombre() %></span>
                                </div>
                            </td>
                            <td class="text-muted"><i class="bi bi-envelope me-1"></i><%= m.getCorreo() %></td>
                            <td><span class="badge bg-success-subtle text-success border border-success-subtle px-3 py-1.5">Activo</span></td>
                            <td class="text-center">
                                <button class="btn btn-sm btn-light border text-primary me-1" data-bs-toggle="modal" data-bs-target="#modalEditarMaestro<%= m.getId_maestro() %>" title="Editar">
                                    <i class="bi bi-pencil-fill"></i>
                                </button>
                                <button class="btn btn-sm btn-light border text-danger" data-bs-toggle="modal" data-bs-target="#modalEliminarMaestro<%= m.getId_maestro() %>" title="Eliminar">
                                    ...
                                    <div class="modal-dialog modal-dialog-centered">
                                        <div class="modal-content border-0 shadow" style="border-radius: 20px; text-align: left;">
                                            <div class="modal-header border-0 pb-0" style="padding: 25px 25px 0 25px;">
                                                <h5 class="modal-title fw-bold text-dark"><i class="bi bi-pencil-square me-2 text-primary"></i>Editar Maestro</h5>
                                            ...
                                            <form action="MaestroServlet" method="POST">
                                                <div class="modal-body p-4">
                                                    ...
                                                    <div class="mb-3">
                                                        ...
                                                        <input type="text" class="form-control bg-light border-0 fw-bold p-2.5" name="nombre" value="<%= m.getNombre() %>" required style="border-radius: 10px; color: var(--text-main);">
                                                    </div>

                                                    <div class="mb-3">
                                                        <label class="form-label fw-bold small text-muted">Correo Electrónico (Login)</label>
                                                        <input type="email" class="form-control bg-light border-0 fw-bold p-2.5" name="correo" value="<%= m.getCorreo() %>" required style="border-radius: 10px; color: var(--text-main);">
                                                    </div>

                                                    <div class="mb-3">
                                                        <label class="form-label fw-bold small text-muted">Nueva Contraseña</label>
                                                        <input type="password" class="form-control bg-light border-0 p-2.5" name="password" placeholder="Dejar en blanco para no cambiarla" style="border-radius: 10px;">
                                                    </div>
                                                </div>
                                        ...
                                        <div class="modal-content border-0 shadow" style="border-radius: 20px;">
                                            <form action="MaestroServlet" method="POST">
                                                <div class="modal-body p-4 text-center">
                                                    ...
                                                    <h4 class="fw-bold mb-2 text-dark">¿Dar de baja?</h4>
                                                    <p class="text-muted small mb-4 px-2">¿Estás seguro de que deseas dar de baja al maestro <br><strong><%= m.getNombre() %></strong>? Perderá el acceso y se desvinculará de sus materias.</p>
    ...
    <div class="modal fade" id="modalNuevoMaestro" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content border-0 shadow" style="border-radius: 20px;">
                <div class="modal-header border-0 pb-0" style="padding: 25px 25px 0 25px;">
                    <h5 class="modal-title fw-bold text-dark"><i class="bi bi-person-plus-fill me-2 text-dark"></i>Registrar Maestro</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form action="MaestroServlet" method="POST">
                    <div class="modal-body p-4">
                        ...
                        <div class="mb-3">
                            <label class="form-label fw-bold small text-muted">Nombre Completo</label>
                            <input type="text" class="form-control bg-light border-0 p-2.5" name="nombre" placeholder="Nombre y Apellidos" required style="border-radius: 10px;">
                        </div>

                        <div class="mb-3">
                            <label class="form-label fw-bold small text-muted">Correo Electrónico (Login)</label>
                            <input type="email" class="form-control bg-light border-0 p-2.5" name="correo" placeholder="ejemplo@peep.com" required style="border-radius: 10px;">
                        </div>
                    ...
                    <div class="modal-footer border-0 p-4 pt-0 d-flex gap-2">
                        <button type="button" class="btn btn-light border fw-bold px-4 flex-grow-1" data-bs-dismiss="modal" style="border-radius: 10px;">Cancelar</button>
                        <button type="submit" class="btn text-white fw-bold px-4 flex-grow-1" style="background-color: #1a1d20; border-radius: 10px;">Guardar Maestro</button>
                    </div>
                </form>
            </div>
        </div>
